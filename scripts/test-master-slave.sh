...
docker exec mt-mysql-master mysql -uroot -pFormosVN@123 -e "
...
docker exec mt-mysql-slave mysql -uroot -pFormosVN@123 -e "
USE millions_transaction;
SELECT * FROM test_replication;
"
...
docker exec mt-mysql-slave mysql -uroot -pFormosVN@123 -e "SHOW SLAVE STATUS\G" | grep -E "(Slave_IO_Running|Slave_SQL_Running|Seconds_Behind_Master)"
